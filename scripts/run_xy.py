...
def load_xy_dataset(n):
    data = onp.load(f"dataset/xy/data/dataset_xy_n{n}.npz")
...
def train_qcl(X_train, y_train, X_test, y_test, n_layers=6, batch_size=64, n_iters=400, seed=42):
    n_qubits = 3
    ...
    for step in range(n_iters):
        ...
        if step % 50 == 0:
            ...
            full_loss = float(np.mean((full_preds - y_train) ** 2))
            print(f"  step {step}/{n_iters}: train MSE = {full_loss:.6f}")
...
def run_fourier_baselines(X_train_np, y_train_np, X_test_np, y_test_np, d, qcl_params, L=6):
    results = []
    ...
    freqs_full_L3 = fourier.all_frequencies(d, 3)
    res_full_L3 = fourier.fourier_fit_and_eval(
        X_train_np, y_train_np, X_test_np, y_test_np,
        freqs_full_L3, label="Full (L=3)"
    )
    if res_full_L3:
        results.append({
            "model": "Fourier full (L=3)",
            "L": 3,
            "freq_count": len(freqs_full_L3),
            "n_params": res_full_L3["n_params"],
            "train_mse": res_full_L3["train_nmse"] * float(onp.var(y_train_np)),
            "train_nmse": res_full_L3["train_nmse"],
            "test_mse": res_full_L3["test_nmse"] * float(onp.var(y_test_np)),
            "test_nmse": res_full_L3["test_nmse"],
        })
...
def run_experiment(output_csv, n_values=None):
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for n in n_values:
            ...
            data = load_xy_dataset(n)
            print(f"Loaded n={n}: {data['X_train_q'].shape[0]} train, {data['X_test_q'].shape[0]} test")
            ...
            print(f"Finished experiment for n={n}. Results appended to {output_csv}")
