...
def load_hubbard_disordered_dataset(n):
    data = onp.load(f"dataset/hubbard_disordered/data/dataset_hubbard_disordered_n{n}.npz")
...
def run_fourier_baselines(X_train_np, y_train_np, X_test_np, y_test_np,
    ...
    # Full Fourier is intractable for d>=4; pre-check column count to avoid
    # enumerating the (2L+1)^d grid.
...
def run_experiment(output_csv, n_values=None):
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for n in n_values:
            print(f"\n{'='*60}")
            print(f"Running disordered (Anderson) Hubbard experiment for n={n} (d={n})")
            print(f"{'='*60}")
            data = load_hubbard_disordered_dataset(n)
            print(f"Loaded n={n}: {data['X_train_q'].shape[0]} train, "
                  f"{data['X_test_q'].shape[0]} test")
            print(f"Observable: double occupancy")
            print(f"Input dimension d={n} (per-site potential eps_i)")
...
if __name__ == "__main__":
    run_experiment("results/hubbard_disordered/hubbard_disordered_results.csv",
                   n_values=[4, 6, 8])
